...
const url = computed(() =>
  cluster.value
    ? `/api/clusters/${cluster.value}/groups?search=${encodeURIComponent(q.value)}&limit=${limit.value}&offset=${offset.value}`
    : '',
)
const { data, pending, error } = await useFetch<{ items: GroupSummary[]; total: number }>(url, {
  watch: [url],
})
  ...
  <section>
    ...
    <template v-else>
      ...
      <table v-else-if="items.length" class="list">
        <thead>
          <tr><th>group</th><th>state</th><th>members</th></tr>
        </thead>
        <tbody>
      ...
      <p v-else class="muted">{{ q ? 'No groups match.' : 'No consumer groups.' }}</p>
    </template>
  </section>
</template>
