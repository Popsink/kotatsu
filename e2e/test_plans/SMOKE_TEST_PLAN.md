...
All data is **fictitious** (`orders`, `events`, `avro-orders`, `qa-group`).

## Scope

| In scope | Out of scope |
|----------|--------------|
| Health, source connectivity, cluster/topic/message read, Avro decode, groups | Load/perf, auth, multi-source, failure injection (see SRC-003) |

## Preconditions

- `docker compose up --build -d` — all services `Up`.
...
| 4 | Topics listed | `GET /api/clusters/demo/topics` | `orders`, `events`, `empty-topic`, `avro-orders` present | TOP-001 |
...
- **Pass**: all 10 steps meet their expected result; no 5xx; UI at `http://localhost:8080` loads and shows the topics.
